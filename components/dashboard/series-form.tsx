...
const availableNiches = [
    { id: 'scary', name: 'Scary Stories', description: 'Chilling tales and urban legends for horror fans.', icon: Ghost, color: 'text-purple-500' },
    { id: 'motivational', name: 'Motivational', description: 'Powerful quotes and stories to inspire your audience.', icon: Flame, color: 'text-orange-500' },
    { id: 'facts', name: 'Mind-Blowing Facts', description: 'Interesting trivia and educational tidbits.', icon: Lightbulb, color: 'text-yellow-500' },
    { id: 'health', name: 'Health & Fitness', description: 'Tips for a better lifestyle and wellness.', icon: HeartPulse, color: 'text-red-500' },
    { id: 'tech', name: 'Tech News', description: 'Latest updates from the world of technology.', icon: Cpu, color: 'text-cyan-500' },
    { id: 'ai', name: 'AI Insights', description: 'Deep dives into artificial intelligence and automation.', icon: BrainCircuit, color: 'text-blue-500' },
];
...
const defaultFormData: SeriesFormData = {
    niche: { type: 'available', selected: null, customDescription: '' },
    identity: { language: Languages[0], voice: null },
    acoustics: { selectedMusicIds: [], volume: 30 },
    visuals: { styleId: null, styleName: '' },
    typography: { styleId: null, styleName: '' },
    schedule: { name: '', duration: '30-50', platforms: [], publishTime: '12:00' },
};

export default function SeriesForm({ initialData, seriesId, mode }: SeriesFormProps) {
    const router = useRouter();
    const [currentStep, setCurrentStep] = useState(0);
    const [formData, setFormData] = useState<SeriesFormData>(initialData || defaultFormData);
    ...
    const handleSubmit = async () => {
        try {
            ...
            toast.success(`Series ${mode === 'create' ? 'scheduled' : 'updated'} successfully!`);
            router.push('/dashboard');
            router.refresh();
        } catch (error: any) {
            console.error(`Failed to ${mode} series:`, error);
            toast.error(error.message || `Failed to ${mode} series. Please try again.`);
        } finally {
    ...
    const updateFormData = (step: string, data: any) => {
        setFormData(prev => ({
            ...prev,
            [step]: { ...prev[step as keyof SeriesFormData], ...data }
        }));
    };
        ...
        <div className="max-w-4xl mx-auto py-8">
            ...
            <div className="glass-dark rounded-[32px] border border-zinc-800/50 p-8 md:p-12 shadow-2xl overflow-hidden relative min-h-[600px] flex flex-col">
                ...
                <div className="flex-1 flex flex-col">
                    {currentStep === 0 && (
                        <div className="space-y-10 relative z-10 flex flex-col h-full">
                            <div className="space-y-2">
                                <h2 className="text-4xl font-black tracking-tight">{mode === 'edit' ? 'Edit' : 'Select'} your Niche</h2>
                    ...
                    {currentStep === 1 && (
                        <div className="flex-1 flex flex-col">
                            <LanguageVoiceSelection formData={formData} updateFormData={updateFormData} />
                    ...
                    {currentStep === 2 && (
                        <div className="flex-1 flex flex-col">
                            <AcousticsSelection formData={formData} updateFormData={updateFormData} />
                    ...
                    {currentStep === 3 && (
                        <div className="flex-1 flex flex-col">
                            <VisualsSelection formData={formData} updateFormData={updateFormData} />
                    ...
                    {currentStep === 4 && (
                        <div className="flex-1 flex flex-col">
                            <TypographySelection formData={formData} updateFormData={updateFormData} />
                    ...
                    {currentStep === 5 && (
                        <div className="flex-1 flex flex-col">
                            <ScheduleSelection formData={formData} updateFormData={updateFormData} />
